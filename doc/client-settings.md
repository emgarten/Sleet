...
| acl                        | A canned (predefined) acl can be set for uploaded files. If a matching canned acl is not found, no acl will be set. By default, no specific canned acl is set and bucket defaults and/or policies are in effect. If the bucket is created by sleet and an acl is set, then the default bucket acl will be set to that acl. |
