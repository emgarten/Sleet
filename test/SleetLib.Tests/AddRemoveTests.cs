...
namespace SleetLib.Tests
{
    public class AddRemoveTests
    {
        ...
        [Fact]
        public async Task AddRemove_AddAndRemovePackageWithIcon()
        {
            // Arrange
            using (var packagesFolder = new TestFolder())
            using (var target = new TestFolder())
            using (var cache = new LocalCache())
            {
                ...
                using (var zip = new ZipArchive(File.OpenRead(zipFile.FullName), ZipArchiveMode.Read, false))
                {
                    ...
                    var iconFile = fileSystem.Get(fileSystem.GetPath("flatcontainer/packagea/1.0.0/icon"));
                    Assert.True(await iconFile.Exists(context.Log, context.Token));

                    await DeleteCommand.RunAsync(context.LocalSettings, context.Source, "packageA", "1.0.0", string.Empty, false, context.Log);

                    iconFile = fileSystem.Get(fileSystem.GetPath("flatcontainer/packagea/1.0.0/icon"));
                    Assert.False(await iconFile.Exists(context.Log, context.Token));
                }
            }
        }
