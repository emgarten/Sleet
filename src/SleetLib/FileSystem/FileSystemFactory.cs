            ...
            foreach (var sourceEntry in sources.Select(e => (JObject)e))
            {
                ...
                if (source.Equals(sourceName, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    else if (type == "minio")
                    {
                        ...
                        var serviceURL = JsonUtility.GetValueCaseInsensitive(sourceEntry, "serviceURL");
                        var serverSideEncryptionMethod = JsonUtility.GetValueCaseInsensitive(sourceEntry, "serverSideEncryptionMethod") ?? "None";
                        var compress = JsonUtility.GetBoolCaseInsensitive(sourceEntry, "compress", false);
                        
                        if (string.IsNullOrEmpty(bucketName))
                        {
                            throw new ArgumentException("Missing 'bucketName' for Minio S3 account.");
                        }

                        if (string.IsNullOrEmpty(region))
                        {
                            throw new ArgumentException("Missing 'region' for Minio S3 account.");
                        }
                        if (string.IsNullOrEmpty(serviceURL))
                        {
                            throw new ArgumentException("Missing 'serviceURL' for Minio S3 account.");
                        }
                        ...
                        config = new AmazonS3Config()
                        {
                            RegionEndpoint = RegionEndpoint.GetBySystemName(region),  // MUST set this before setting ServiceURL and it should match the `MINIO_REGION` environment variable.
                            ServiceURL = serviceURL,
                        ...
                        {
                            // Find the default path
                            pathUri = MinioS3Utility.GetBucketPath(bucketName, config.ServiceURL);
                        }
